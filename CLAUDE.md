...
2. **TEV Consulta** (ferramenta privada). Fontes em `tev-bibliografia/consulta` (`c1`, `c2`, `c3` e as imagens); `bash consulta/build-consulta.sh` gera `TEV Consulta.html` na raiz do repositório privado. As alterações à Consulta fazem-se e ficam apenas no `tev-bibliografia`. Nunca vai para o `TEV` até o Tiago dizer "põe a Consulta online". Nessa altura, e só nessa altura, copia-se o ficheiro construído para `TEV/consulta.html`, faz-se commit no `main` do `TEV` e a ferramenta fica em https://tiagojgcc.github.io/TEV/consulta.html. Enquanto essa ordem não for dada, o `TEV` não contém nenhum ficheiro da Consulta.
   - Depois de qualquer alteração à Consulta: `bash consulta/build-consulta.sh` (gera o ficheiro, verifica travessões e sintaxe), verificar a renderização em Playwright (400 px, sem erros de página nem transbordo) e correr `NODE_PATH=$(npm root -g) node consulta/testes/casos.js`, comparando com `consulta/testes/casos-esperado.md`. Uma diferença só é aceitável se for intencional, e nesse caso atualiza-se o esperado no mesmo commit. Commit direto no `main` do `tev-bibliografia`.
   - Regras da Consulta (não regredir): os meses desde o evento são obrigatórios no ecrã inicial; "TEV recorrente" é uma caixa modificadora no ecrã inicial (salta a Tabela 11 e entra na categoria de risco alto, ESC Classe I B); a TVS tem via curta (`c_tvs`: resolvida dá alta, recorrente vai ao estudo etiológico) e não passa pela Tabela 11 nem pelo score hemorrágico; o risco hemorrágico é o **VTE-BLEED** (pontuação e corte ≥2 da Tabela suplementar 14 da ESC 2019), sem outros scores; a dose reduzida só é elegível com ≥6 meses e nunca em SAF nem em TEV recorrente; o botão de AVK só aparece no SAF; "Adiar" marca reavaliação em 3 meses; os D-dímeros seriados após a suspensão são aos dias 15, 30 e 60 (DULCIS); cada sugestão de duração leva a classe da ESC.
   - Formato da nota da Consulta: título `# Consulta de TEV: <evento>[ recorrente], N meses do evento`; linhas "- Evento índex: <fator> > risco de recorrência <risco> (Tabela 11 da ESC 2019).", "- Risco hemorrágico: VTE-BLEED N pontos, baixo/alto.", "- Decisão: ...", "- Estudo etiológico: ...", "- Complicações: ...", "- Plano: ...". Na TVS a linha é "- Evento: ...". Usar `fim()`, `plural()` e `minusc()` (inicial minúscula que respeita siglas).
